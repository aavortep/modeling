...
class Processor(Generator):
    ...
    def process(self):
        if self.queue_size > 0:
            ...
            if numpy_random.random_sample() <= self.reenter_prob and \
                    self.reentered_requests < self.reenter_prob * COUNT:
...
class NormalDistribution:
    ...
    def generate(self):
        return norm.rvs(loc=self.m, scale=self.sigma, size=1)[0]


class Model:
    def __init__(self, uniform_a, uniform_b, norm_m, norm_sigma, reenter_prop):
        self.generator = Generator(UniformDistribution(uniform_a, uniform_b))
        self.processor = Processor(NormalDistribution(norm_m, norm_sigma), reenter_prop)
...
if __name__ == '__main__':
    a, b = map(int, input("Введите (через пробел) a и b для равномерного распределения: ").split())

    if a >= b:
        print("Параметр a должен быть меньше параметра b")
        sys.exit(0)

    m, sigma = map(int, input("Введите (через пробел) m и σ для нормального распределения: ").split())

    if sigma <= 0:
        print("σ должна быть больше 0")
        sys.exit(0)

    repeat_probality = float(input("Введите максимальное значение процента задач, которые возвращаются "
                                   "в очередь после обработки (в %): "))
    if 0 <= repeat_probality <= 100:
    ...
    model = Model(a, b, m, sigma, repeat_probality)
    result1 = model.event_based_system(total_tasks)
    model2 = Model(a, b, m, sigma, repeat_probality)
    result2 = model2.time_based_modelling(total_tasks, step)

    table = PrettyTable()
    table.add_column("Метод", ["Событийный", "Дельта t"])
